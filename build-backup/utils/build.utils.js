...
const BuildUtils = {
  createEntryPoint(packageName, installPath, options) {
    const entryPath = path_1.default.join(
      installPath,
      options.entryFilename || 'index.ts'
    )
    let importStatement
    if (options.esm) {
  ...
  async compilePackage({
    ...
    const nodeFS = new fs_2.NodeFS()
    await (0, common_utils_1.updateProjectPeerDependencies)(
      installPath,
      Object.fromEntries(
    ...
    try {
      ...
      console.log('got', Object.getOwnPropertyNames(bundler))
      ...
      telemetry_utils_1.default.compilePackage(name, true, startTime, {
        minifier,
      })
    } catch (err) {
      console.log('Parcel failed becase ', err)
      console.log(err.diagnostics[0].codeFrames[0].codeHighlights)
      telemetry_utils_1.default.compilePackage(
        name,
        false,
        startTime,
        { minifier },
  ...
  _parseMissingModules(errors) {
    ...
    const missingModules = missingModuleErrors.map(err => {
      ...
      if (!matches) {
        throw new CustomError_1.UnexpectedBuildError(
  ...
  async buildPackage({ name, installPath, externals, options }) {
    ...
    try {
      ...
      const assetStats = assets.filter(asset => !!asset.file).map(getAssetStats)
      return {
        assets: assetStats || [],
  ...
  async buildPackageIgnoringMissingDeps({
    ...
    try {
      ...
      telemetry_utils_1.default.buildPackage(
        name,
        true,
        buildStartTime,
        Object.assign(Object.assign({}, options), { buildIteration })
      )
      return buildResult
    } catch (e) {
      buildIteration++
      console.log(
        'In buildPackageIgnoringMissingDeps, I got',
        e,
        e instanceof CustomError_1.MissingDependencyError
      )
      if (
        e instanceof CustomError_1.MissingDependencyError &&
        ...
        const newExternals = Object.assign(Object.assign({}, externals), {
