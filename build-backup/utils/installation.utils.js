...
const wrapPackCommand = packagePath =>
  `$(npm pack --ignore-scripts ${packagePath} | tail -1)`
const InstallationUtils = {
  getInstallPath(packageName) {
    const id = shortid_1.default.generate().slice(0, 3)
    return path_1.default.join(
      config_1.default.tmp,
      'packages',
      (0, sanitize_filename_1.default)(`build-${packageName}-${id}`)
    )
  },
  async preparePath(packageName) {
    ...
    await fs_1.promises.writeFile(
      path_1.default.join(installPath, 'yarn.lock'),
      ''
    )
    await fs_1.promises.writeFile(
      path_1.default.join(installPath, 'package.json'),
      JSON.stringify({
        source: './index.ts',
        main: './dist/main.js',
        targets: {
  ...
  async installPackage(packageString, installPath, installOptions) {
    ...
    try {
      ...
      telemetry_utils_1.default.installPackage(
        packageString,
        true,
        installStartTime,
        installOptions
