...
function checkBundleSize(localSize: number, originalSize: number) {
  ...
  const isSignificant = (sizeDiff >= 10240) && (sizePercent > 10 || sizeDiff > 20480);
...
export function calculateDependencyShares(
  ...
  deps.forEach(dep => {
    const percentage = totalWeight > 0 
      ? ((dep.approximateSize || 0) / totalWeight) * 100 
      : 0;
 ...
 */
export function isSignificantDifference(metrics: ComparisonMetrics): ComparisonResult {
  ...
  // Rule 2: Exports
  const exportCheck = checkExports(metrics.localExports, metrics.originalExports);
  if (exportCheck.flag) {
  ...
  const depCountCheck = checkDependencyCount(metrics.localDeps, metrics.originalDeps);
