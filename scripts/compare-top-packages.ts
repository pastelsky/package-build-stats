...
const ORIGINAL_DIR = process.env.ORIGINAL_DIR || '../package-build-stats-original';
...
const packages = JSON.parse(fs.readFileSync(path.join(LOCAL_DIR, 'scripts/top-100-recent-packages.json'), 'utf8'));
...
function runAnalysis(dir: string, pkg: string, label: string): Promise<AnalysisResult | null> {
    ...
    return new Promise(resolve => {
        ...
        child.on('close', () => {
            ...
            if (match) {
                ...
                console.log(`    ${colorize('✓', colors.green)} ${label}: ${colorize(formatTime(elapsed), colors.cyan)}`);

                // Capture build artifacts (install path) if available
                if (parsedResult.installPath && fs.existsSync(parsedResult.installPath)) {
                    try {
                        ...
                        fs.cpSync(parsedResult.installPath, destBuildPath, { recursive: true });

                        // Clean up the temp dir since we used debug: true
                        fs.rmSync(parsedResult.installPath, { recursive: true, force: true });
                    } catch (e) {
                ...
                const elapsed = Date.now() - startTime;
                console.log(`    ${colorize('✗', colors.red)} ${label}: ${colorize('FAILED', colors.red)} (${formatTime(elapsed)})`);
                if (stderr) {
                    fs.appendFileSync(path.join(logDir, 'stderr.log'), `\n[Runner stderr]: ${stderr}`);
                }
                resolve(null);
            }
        });
...
function compareExports(local: any[], original: any[]) {
    ...
    return { added, removed, sizeDiff, localCount: local.length, originalCount: original.length };
}
...
function getRatingColor(rating: string): string {
    switch (rating) {
        ...
        case 'Significant': return colors.red;
...
function getDepsDescription(depsComp: ReturnType<typeof compareDeps>): string {
    ...
    if (depsComp.added.length > 0) {
        parts.push(`+${depsComp.added.length} new: ${depsComp.added.slice(0, 2).join(', ')}${depsComp.added.length > 2 ? '...' : ''}`);
    }
    if (depsComp.removed.length > 0) {
        parts.push(`-${depsComp.removed.length} removed: ${depsComp.removed.slice(0, 2).join(', ')}${depsComp.removed.length > 2 ? '...' : ''}`);
    }
...
function getExportsDescription(exportsComp: ReturnType<typeof compareExports>): string {
    const parts: string[] = [];
    if (exportsComp.added.length > 0) {
        parts.push(`+${exportsComp.added.length} new: ${exportsComp.added.slice(0, 2).join(', ')}${exportsComp.added.length > 2 ? '...' : ''}`);
    }
    if (exportsComp.removed.length > 0) {
        parts.push(`-${exportsComp.removed.length} removed: ${exportsComp.removed.slice(0, 2).join(', ')}${exportsComp.removed.length > 2 ? '...' : ''}`);
    }
    ...
    if (parts.length === 0) {
        if (exportsComp.localCount === 0 && exportsComp.originalCount === 0) {
            ...
            parts.push(`local: ${exportsComp.localCount}, original: ${exportsComp.originalCount}`);
        }
    }
    return parts.join(', ');
}
...
async function main() {
    ...
    console.log(colorize('║', colors.cyan) + colorize('  PACKAGE BUILD STATS - COMPARISON REPORT  ', colors.bold, colors.cyan) + colorize('               ║', colors.cyan));
    console.log(colorize('╚══════════════════════════════════════════════════════════╝', colors.cyan));
    console.log('');
    console.log(`  ${colorize('Packages to analyze:', colors.dim)} ${subset.length}`);
    ...
    const processPackage = async (pkg: string, index: number) => {
        const pkgSafeName = pkg.replace(/\//g, '__');
        console.log('');
        console.log(`${colorize(`[${startIdx + index + 1}/${endIdx}]`, colors.bold, colors.blue)} ${colorize(pkg, colors.bold)}`);

        const local = await runAnalysis(LOCAL_DIR, pkg, 'Local');
        const original = await runAnalysis(ORIGINAL_DIR, pkg, 'Original');

        if (!local || !original) {
            console.log(`    ${colorize('→ Result:', colors.dim)} ${colorize('FAILED', colors.bgRed, colors.bold)}`);
            printSeparator();
        ...
        const depsComp = compareDeps(local.dependencySizes || [], original.dependencySizes || []);
        const exportsComp = compareExports(local.exports || [], original.exports || []);
        ...
        const sizeStatus = comparisonResult.details.sizeFlag ? 'Significant' : 'Match';
        const depsStatus = comparisonResult.details.depCountFlag || comparisonResult.details.depShareFlag ? 'Significant' : 'Match';
        const exportsStatus = comparisonResult.details.exportFlag ? 'Significant' : 'Match';
        ...
        console.log(`    ${colorize('→ Size:', colors.dim)} Local ${colorize(formatBytes(local.size), colors.cyan)} | Original ${colorize(formatBytes(original.size), colors.cyan)} | ${colorize(sizeStatus, getRatingColor(sizeStatus))}`);
        console.log(`    ${colorize('→ Deps:', colors.dim)} ${colorize(depsStatus, getRatingColor(depsStatus))} (${depsDesc})`);
        console.log(`    ${colorize('→ Exports:', colors.dim)} ${colorize(exportsStatus, getRatingColor(exportsStatus))} (${exportsDesc})`);
        
        // Show comparison rules results
        if (comparisonResult.reasons.length > 0) {
            console.log(`    ${colorize('→ Rules:', colors.dim)} ${colorize(comparisonResult.reasons.join(', '), colors.magenta)}`);
        }

        if (localTimeMs && originalTimeMs) {
            const speedDiff = originalTimeMs - localTimeMs;
            const speedPercent = Math.abs((speedDiff / originalTimeMs) * 100).toFixed(1);
            const isFaster = speedDiff > 0;
            const speedLabel = isFaster ? 'faster' : 'slower';
            const speedColor = isFaster ? colors.green : colors.red;
            console.log(`    ${colorize('→ Speed:', colors.dim)} ${colorize(`${speedPercent}% ${speedLabel}`, speedColor)} (${formatTime(localTimeMs)} vs ${formatTime(originalTimeMs)})`);
        }
...
${comparisonResult.details.depShareDifferences && comparisonResult.details.depShareDifferences.length > 0 ? `
...
`;
        ...
        fs.writeFileSync(path.join(pkgReportDir, 'comparison.json'), JSON.stringify({ local, original }, null, 2));
    ...
    console.log(colorize('║', colors.magenta) + colorize('                    FINAL SUMMARY                         ', colors.bold, colors.magenta) + colorize('║', colors.magenta));
    console.log(colorize('╚══════════════════════════════════════════════════════════╝', colors.magenta));
    console.log('');
    console.log(`  ${colorize('Success:', colors.green)} ${successCount}/${subset.length} packages`);
    console.log(`  ${colorize('Failed:', colors.red)} ${failCount}/${subset.length} packages`);
    console.log('');
    console.log(`  ${colorize('Total Local Time:', colors.dim)} ${formatTime(totalLocalTime)}`);
    console.log(`  ${colorize('Total Original Time:', colors.dim)} ${formatTime(totalOriginalTime)}`);
    if (totalOriginalTime > 0) {
        const overallSpeedup = ((totalOriginalTime - totalLocalTime) / totalOriginalTime * 100).toFixed(1);
        console.log(`  ${colorize('Overall Speedup:', colors.bold)} ${colorize(overallSpeedup + '%', colors.green)}`);
    }
    console.log('');
...
`;
    ...
    for (const s of summary) {
        if (s.status === 'FAILED') {
            ...
            const sizeEmoji = s.sizeStatus === 'Match' ? '✅' : s.sizeStatus === 'Medium' ? '⚠️' : '❌';
            const depsEmoji = s.depsStatus === 'Match' ? '✅' : s.depsStatus === 'Medium' ? '⚠️' : '❌';
            const exportsEmoji = s.exportsStatus === 'Match' ? '✅' : s.exportsStatus === 'Medium' ? '⚠️' : '❌';
...
`;
    ...
    console.log(`${colorize('✓', colors.green)} Reports generated in ${colorize('reports/', colors.cyan)}`);
    console.log('');
}
