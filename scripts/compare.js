...
async function cmdTest(packages) {
  ...
  log(
    `Testing ${packages.length} package(s) (${CONCURRENCY} at a time)`,
    'green',
  )
  console.log()
  ...
  const masterBuildExists = fs.existsSync(
    path.join(MASTER_CACHE_DIR, 'build', 'index.js'),
  )
  ...
  if (masterBuildExists) {
    ...
    log(
      'Cloning and building master branch (this may take 1-2 minutes, only needed once)...',
      'yellow',
    )
    try {
      ...
      execSync(
        `git clone --branch master --single-branch "${PROJECT_ROOT}" "${MASTER_CACHE_DIR}"`,
  ...
  return new Promise(resolve => {
    ...
    const masterProc = spawn(
      ...
      [
        ...
        path.join(MASTER_CACHE_DIR, 'test-master.js'),
        packageName,
      ],
    ...
    const locProc = spawn(
      'node',
      [path.join(resultDir, 'test-local.js'), packageName],
...
async function testPackagesInBatches(packages, resultDir) {
  ...
  for (let i = 0; i < packages.length; i += CONCURRENCY) {
    ...
    console.log()
    log(
      `Progress: ${Math.min(i + CONCURRENCY, packages.length)}/${packages.length} packages tested`,
      'blue',
    )
    console.log()
  }
...
`
  ...
  results.forEach(({ safeName }) => {
    ...
    if (
      master.tests?.getPackageStats?.success &&
      loc.tests?.getPackageStats?.success
    ) {
...
function printSummary(results, resultDir, totalTime) {
  ...
  results.forEach(({ package: pkg, safeName, masterTime, localTime }) => {
        ...
        if (Array.isArray(val1) && Array.isArray(val2)) {
          ...
          if (val1.length !== val2.length) {
            diffs.push({
              ...
              master: String(val1.length),
              local: String(val2.length),
              change: '',
            })
          }
            ...
            for (let i = 0; i < maxLen; i++) {
              if (i < val1.length && i < val2.length) {
                if (
                  ...
                  diffs.push(
                    ...deepCompare(val1[i], val2[i], `${currentPath}[${i}]`),
                  )
                } else if (val1[i] !== val2[i]) {
                  diffs.push({
                    path: `${currentPath}[${i}]`,
                    master: formatValue(val1[i]),
                    local: formatValue(val2[i]),
                    change: '',
                  })
                }
              }
    ...
    if (master.tests && loc.tests) {
      ...
      deepDiffs.forEach(diff => {
        ...
        differences.push(
          `${path}: ${diff.master} → ${diff.local}${diff.change ? ' (' + diff.change + ')' : ''}`,
        )
      })
    }
  ...
  log(
    ...
    summary.succeeded.length > 0 ? 'green' : 'reset',
  )
  log(
    `✗ Failed: ${summary.failed.length}`,
    summary.failed.length > 0 ? 'red' : 'reset',
  )
  console.log()

  // Print identical vs different
  log(
    `═ Identical results: ${summary.identical.length}`,
    summary.identical.length > 0 ? 'green' : 'reset',
  )
  log(
    `≠ Different results: ${summary.different.length}`,
    summary.different.length > 0 ? 'yellow' : 'reset',
  )
  console.log()
  ...
  if (summary.failed.length > 0) {
    ...
    summary.failed.forEach(({ pkg, reason, masterTime, localTime }) => {
      ...
      if (masterTime || localTime) {
        log(
          `    Time: master=${masterTime || 'N/A'}s, local=${localTime || 'N/A'}s`,
          'reset',
        )
      }
    })
  ...
  if (summary.different.length > 0) {
    ...
    summary.different.forEach(({ pkg, differences, masterTime, localTime }) => {
      ...
      if (differences.length > 0) {
        ...
        log(header, 'bright')
        log(
          `  ${'─'.repeat(metricWidth)}─┼─${'─'.repeat(masterWidth)}─┼─${'─'.repeat(localWidth)}─┼─${'─'.repeat(changeWidth)}`,
          'reset',
        )
